...
describe("NumberParameters.param.spec.ts / Default commands", () => {
	...
	describe("NumberParameter", () => {
		...
		it("Single Parameter", () => {
			...
			expect(test(r, "test cmd -10")).to.eq(true, "Negative number.");
			expect(test(r, "test cmd 1337")).to.eq(true, "Positive number.");
		});
		...
		it("Positive Parameter", () => {
			...
			expect(test(r, "test cmd 10")).to.eq(true, "Positive number.");
			expect(test(r, "test cmd -10")).to.eq(false, "Negative number.");
		});
		...
		it("Negative Parameter", () => {
			...
			expect(test(r, "test cmd 10")).to.eq(false, "Positive number.");
			expect(test(r, "test cmd -10")).to.eq(true, "Negative number.");
		});
		...
		it("Double Parameter", () => {
			...
			expect(test(r, "test cmd -10 1337")).to.eq(
				true,
				"Negative number followed by a positive number."
			);
			expect(test(r, "test cmd 1337 -10")).to.eq(
				true,
	...
	describe("FractionParameter", () => {
		...
		it("Single Parameter", () => {
			...
			expect(test(r, "test cmd -10.144")).to.eq(true, "Negative number.");
			expect(test(r, "test cmd 1337.28")).to.eq(true, "Positive number.");
		});
